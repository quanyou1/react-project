...
class InforSearch extends React.Component {
  constructor(props) {
   ...
   getData(page) {
		...
		var webSite = "http://www.szspia.org";
   		...
   		fetchJsonp("http://www.szspia.org/index.php?m=Home&c=homeapi&a=homesearch&key=QUgFYq4L5n89ZG2Ab&callback=userapi&catid=1&limit=10&keyword=" + `${this.props.match.params.dataSource}` + "&p=" + page).then(response => response.json()).then(inforList => {
			this.setState({
	...
	componentDidMount() {
		...
		fetchJsonp("http://www.szspia.org/index.php?m=Home&c=homeapi&a=homesearch&key=QUgFYq4L5n89ZG2Ab&callback=userapi&catid=1&limit=10&keyword=" + `${this.props.match.params.dataSource}` + "&p=" +this.state.current, myFetchOptions).then(response => response.json()).then(inforList => {
			this.setState({
                inforList: inforList.data,
  ...
  createMarkup() {
    return {__html: this.state.inforList.title};
  };
  ...
  render () {
    ...
    console.log(this.state.inforList.title);
    ...
    let ListDate = inforList && inforList.length || 0 ?
        inforList.map((inforItem, index) => {
                ...
                return <li>
                    <Link to={`/inforDel/协会动态/${inforItem.id}`}  id={inforItem.id}>
                ...
                <div className={detailCss.detail_container}>
                    <div className={detailCss.detail_wrapper}>
                        <div className={detailCss.detail_title}>
                            <Link to={''}>深安协 ></Link><span>搜素：{this.props.match.params.dataSource}</span>
                        </div>
