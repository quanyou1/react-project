...
import {Icon, Pagination, Collapse} from 'antd';
  ...
  constructor(props) {
   ...
   getData(page) {
		...
		var webSite = "http://www.szspia.org";
   		...
   		fetchJsonp("http://www.szspia.org/index.php?m=Home&c=homeapi&a=homesearch&key=QUgFYq4L5n89ZG2Ab&callback=userapi&catid=2&limit=10&keyword=" + `${this.props.match.params.dataSource}` + "&p=" + page).then(response => response.json()).then(inforList => {
			this.setState({
   ...
   onScreen = (unit, value, addtime) => {
    ...
    getScreenData(unit, value, addtime) {
        ...
        fetchJsonp("http://www.szspia.org/index.php?m=Home&c=homeapi&a=homesearch&key=QUgFYq4L5n89ZG2Ab&callback=userapi&limit=10&catid=2&keyword=" + `${this.props.match.params.dataSource}` + "&p=1&level=" + unit + "&city=" + value + "&addtime=" + addtime).then(response => response.json()).then(inforList => {
	...
	componentDidMount() {
        ...
        fetchJsonp("http://www.szspia.org/index.php?m=Home&c=homeapi&a=homesearch&key=QUgFYq4L5n89ZG2Ab&callback=userapi&catid=2&limit=10&keyword=" + `${this.props.match.params.dataSource}` + "&p=" +this.state.current, myFetchOptions).then(response => response.json()).then(inforList => {
            this.setState({
              inforList: inforList.data,
              inforWhole: inforList.count
            });
                    ...
                    <div className={detailCss.detail_wrapper} style={{marginRight: '0px'}}>
                        ...
                        <div className={detailCss.member_search}>
                            <div className={detailCss.search_input}>
                                <AutoComplete
                                    ...
                                    placeholder={this.props.match.params.dataSource}
                                />
                                <Link target="_blank" to={`/infor_search/2/${this.state.dataSource}`} >搜索</Link>
                            </div>
                        ...
                        <div className={detailCss.member_wrapper}>
                            <div className={detailCss.listTitle}>
                                ...
                                <p className={detailCss.more_in}>所在地区</p>
                                <p>会员级别</p>
                                <p>加入时间</p>
                            </div>
